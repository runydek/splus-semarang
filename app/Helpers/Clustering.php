...
class Clustering {
     ...
     */
    public function __construct($centeroids = array(), $attributes = array()){
        ...
        try{
            ...
            for($cluster = 0;$cluster < count($results); $cluster++){
                for($record = 0;$record < count($results[$cluster]); $record++){
                    echo Sprintf('Cluster %s Record %s : %s <br>', $record+1, $cluster+1, round($results[$cluster][$record]), 2)."\n";
                }
            }
    ...
    public function determineCluster($clusterGroupings = array()) : array {
        ...
        for($position = 0; $position < count($resultsOfDetermine); $position++){
            echo Sprintf("Position %s with value %s <br>", $resultsOfDetermine[$position]['position'], round($resultsOfDetermine[$position]["value"], 2))."\n";
        }
